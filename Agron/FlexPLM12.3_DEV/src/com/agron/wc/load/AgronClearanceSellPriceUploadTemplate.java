...
public class AgronClearanceSellPriceUploadTemplate {
	...
	public static final Logger logger = LogManager.getLogger(AgronClearanceSellPriceUploadTemplate.class);
	public static String UPLOAD_ATTRIBUTE_LIST=LCSProperties.get("com.agron.wc.load.clearanceSellPrice.excelUploadAttributes");
	public static String ignoreBlankValue=LCSProperties.get("com.agron.wc.load.clearanceSellPrice.ignoreBlankValue");
	...
	public AgronClearanceSellPriceUploadTemplate(WTUser userObj, String dataFile, String dataLoadType) {
	...
	public void loadData(ArrayList keyList, ArrayList dataList, boolean ignoreBlankValue) throws WTException, WTPropertyVetoException {
	 ...
	 if(dataList.isEmpty() && dataList.size() == 0) {
						...
						if (ft.getAttVariableType().equals("boolean") && (!"".equals(tempRow.get(j).toString().trim()) || ft.isAttRequired() )) {
							...
							if(!bolAttribute.trim().equalsIgnoreCase("TRUE") && !bolAttribute.trim().equalsIgnoreCase("FALSE")){
								...
								if(isError) {
									...
									loggerRowData.append(AgronClearanceSellPriceUploadTemplate.LOGGER_DELIMITER);
		...
		logger.info("!!>>> Clearance Sell Price Load Summary>>>>>>>>>");
	...
	private void notifyPLMUser(boolean isValidFile,  ArrayList dataList, int successCount, int failedCount ) throws WTException {
		...
		if(isValidFile) {
			emailContent.append("Clearance Sell Price upload is successful.");
			}else {
				emailContent.append("Clearance Sell Price Load is failed.");
				emailContent.append("There is no data in the excel file So please add the data in the excel file");
			}
		String emailSubject = "Clearance Sell Price Upload Summary";
		emailContent.append(System.getProperty("line.separator"));
	...
	public static void appendRowLoadStatus(StringBuffer logEntryLine, String status, String printMsg) {
		...
		logEntryLine.append(AgronClearanceSellPriceUploadTemplate.LOGGER_DELIMITER);
