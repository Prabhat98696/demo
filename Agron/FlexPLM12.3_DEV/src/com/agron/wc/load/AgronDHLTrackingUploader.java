...
public class AgronDHLTrackingUploader {
	...
	public static final Logger logger = LogManager.getLogger(AgronDHLTrackingUploader.class);
	public static String UPLOAD_ATTRIBUTE_LIST=LCSProperties.get("com.agron.wc.load.DHLTracking.excelUploadAttributes");
	...
	public AgronDHLTrackingUploader(WTUser userObj, String dataFile, String dataLoadType) {
	...
	public void loadData(ArrayList keyList, ArrayList dataList, boolean ignoreBlankValue) throws WTException, WTPropertyVetoException {
		...
		logger.info("DHL Tracking Uploader");
	 ...
	 if(dataList.isEmpty() && dataList.size() == 0) {
		...
		while(dataListItr.hasNext()){
			...
			String skuArticle=tempRow.get(1).toString();
			//String colorwayArticle=tempRow.get(2).toString();

			FlexType Product_flexType = FlexTypeCache.getFlexTypeFromPath("Product");
			FlexType SKU_FlexType = FlexTypeCache.getFlexTypeFromPath("Product\\Adidas");

			HashMap<String, String> criteriaMap = new HashMap<String, String>();
			criteriaMap.put(SKU_FlexType.getAttribute("agrArticle").getSearchCriteriaIndex(), skuArticle);
			
			logger.debug("::::criteriaMap::>>"+criteriaMap);

			SearchResults skuObjsRes = new LCSSKUQuery().findSKUsByCriteria(criteriaMap, SKU_FlexType, null, null, null);
			...
			if(skuObj.size() > 0) {
				...
				logger.debug("ERROR : SKU Object not found For Article "+skuArticle);
						...
						if (ft.getAttVariableType().equals("boolean") && (!"".equals(tempRow.get(j).toString().trim()) || ft.isAttRequired() )) {
							...
							if(!bolAttribute.trim().equalsIgnoreCase("TRUE") && !bolAttribute.trim().equalsIgnoreCase("FALSE")){
								...
								if(isError) {
									...
									loggerRowData.append(AgronDHLTrackingUploader.LOGGER_DELIMITER);
		...
		logger.info("******DHL Tracking Uploader Summary**********");
	...
	private void notifyPLMUser(boolean isValidFile,  ArrayList dataList, int successCount, int failedCount ) throws WTException {
		...
		if(isValidFile) {
			emailContent.append("DHL Tracking Data Load is successful.");
			}else {
				emailContent.append("DHL Tracking Data Load is failed.");
				emailContent.append("There is no data in the excel file So please add the data in the excel file");
			}
		String emailSubject = "DHL Tracking Upload Summary";
		emailContent.append(System.getProperty("line.separator"));
	...
	public static void appendRowLoadStatus(StringBuffer logEntryLine, String status, String printMsg) {
		...
		logEntryLine.append(AgronDHLTrackingUploader.LOGGER_DELIMITER);
