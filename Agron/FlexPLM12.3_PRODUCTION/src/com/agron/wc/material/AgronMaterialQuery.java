...
public class AgronMaterialQuery extends LCSQuery implements Externalizable {
	...
	public SearchResults findMaterialsByCriteria(Map criteria, FlexType type, Collection attCols, FiltersList filter,
		...
		List<String> agrRefkeyWordsList= new ArrayList<String>();
		String tempString = "";
		...
		//Logic for comma separated agron ref# number
		if (FormatHelper.hasContent(String.valueOf(criteria.get("LCSMATERIAL_ptc_str_7")))) {
			
			String agrRefvalue = String.valueOf(criteria.get("LCSMATERIAL_ptc_str_7"));
			// Quick Search with comma (,) separated value
			// Check whether string contains (,) if contains the split into , separated 
			String[] agrRefelements = agrRefvalue.split(",");
			for(int i = 0; i < agrRefelements.length; i++) {
				agrRefkeyWordsList.add(agrRefelements[i].trim());
			}	
		}
		...
		if (criteria != null) {
			...
			if (!agrRefkeyWordsList.isEmpty() && agrRefkeyWordsList.size() == 1) {
				//If only 1 keyword in agronRef# then return OOTB statement
			}
			else if (!agrRefkeyWordsList.isEmpty() && agrRefkeyWordsList.size() > 1) {
				// If multiple keywords in agronRef# then return statement
				// with append criteria for multiple keyword
				addAgrrefCriteriaForCommaSeparated(statement, type, "Material", "agrMaterialRefNumber", (String) criteria.get("LCSMATERIAL_ptc_str_7"), agrRefkeyWordsList);	
				criteria.remove("LCSMATERIAL_ptc_str_7",(String) criteria.get("LCSMATERIAL_ptc_str_7"));
			}
	 ...
	 * @param materialObj
	 * @param agrRefAttKey
	 * @param agrRefcriteriaStr
	 * @param agrRefkeyWordsList
	 * @return
	 * @throws LCSException
	 */
	public static PreparedQueryStatement addAgrrefCriteriaForCommaSeparated(PreparedQueryStatement statement, FlexType type, String materialObj,
			String agrRefAttKey, String agrRefcriteriaStr, List<String> agrRefkeyWordsList) throws LCSException {

		if (!FormatHelper.hasContent(agrRefcriteriaStr)) {
			return statement;
		} else {
			String agrRefAttributes = "name,agrMaterialRefNumber";
					//LCSProperties.get("jsp.main.header.quickSearchAttributes." + materialObj);
	

	if (agrRefAttributes != null && !"".equalsIgnoreCase(agrRefAttributes)) {
		try {
			StringTokenizer attKeys = new StringTokenizer(agrRefAttributes, ",");
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Configurable Attributes : " + agrRefAttributes);
			}
				...
				while (true) {
					...
					try {
						...
						if (FormatHelper.hasContent(attScope) || FormatHelper.hasContent(attLevel)) {
							label182 : {
								if (materialObj.equalsIgnoreCase("PLACEHOLDER")
										&& !attScope.equalsIgnoreCase("PLACEHOLDER")) {
									throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
											"attNotConfigured_MSG", objA);
								}

								if (!materialObj.equalsIgnoreCase("SKU") || attScope.equalsIgnoreCase("PRODUCT")
										&& (attLevel.equalsIgnoreCase("SKU")
												|| attLevel.equalsIgnoreCase("PRODUCT-SKU"))) {
									if (!materialObj.equalsIgnoreCase("PRODUCT")
											|| attScope.equalsIgnoreCase("PRODUCT")
													&& (attLevel.equalsIgnoreCase("PRODUCT")
															|| attLevel.equalsIgnoreCase("PRODUCT-SKU"))) {
										if (materialObj.equalsIgnoreCase("MATERIAL")
												&& !attScope.equalsIgnoreCase("MATERIAL")) {
											throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
													"attNotConfigured_MSG", objA);
										}

										if (materialObj.equalsIgnoreCase("ORDERCONFIRMATION")
												&& !attScope.equalsIgnoreCase("HEADER_SCOPE")) {
											throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
													"attNotConfigured_MSG", objA);
										}

										if (materialObj.equalsIgnoreCase("PLAN")
												&& !attScope.equalsIgnoreCase("PLAN_SCOPE")) {
											throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
													"attNotConfigured_MSG", objA);
										}

										if (materialObj.equalsIgnoreCase("RFQ")
												&& !attScope.equalsIgnoreCase("REQUEST_HEADER_SCOPE")) {
											throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
													"attNotConfigured_MSG", objA);
										}

										if ((materialObj.equalsIgnoreCase("SAMPLE")
												|| materialObj.equalsIgnoreCase("SAMPLEPRODUCT")
												|| materialObj.equalsIgnoreCase("SAMPLEMATERIAL"))
												&& !attScope.equalsIgnoreCase("SAMPLE")) {
											throw new LCSException("com.lcs.wc.resource.QuickSearchRB",
													"attNotConfigured_MSG", objA);
										}

										if (materialObj.equalsIgnoreCase("SIZING")
					...
					if (ACLHelper.hasViewAccess(group)) {
						...
						for (int i = 0; i < agrRefkeyWordsList.size(); i++) {
							statement.addPossibleSearchCriteria(
									new QueryColumn(typeName, attribute.getColumnName()), agrRefkeyWordsList.get(i),
									true);
							if (i != agrRefkeyWordsList.size() - 1) {
								statement.appendOrIfNeeded();
							}
						}
						// END 
