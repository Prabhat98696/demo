...
public class AgronDataHelperUtil {
	
	private static final Logger LOGGER = LogManager.getLogger(AgronDataHelperUtil.class);
	...
	public static String getDataFromKey(WTObject obj, String key, boolean needValue2) throws WTException {
		LOGGER.info("!! Method getDataFromKey START key>>>>>"+key);
		String value = "";
		FlexType type;
		...
		if(FormatHelper.hasContent(value) && value.contains(",") && !(key.contains("agrArticle"))){
			value = value.replaceAll(",", "|"); // This logic is for legacy articles with comma separation.
		}
		...
		LOGGER.info("!! Method getDataFromKey END>>>>>");
		return value;
