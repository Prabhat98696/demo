...
public class AgronLogEntryManagement {

	private static final Logger LOGGER = LogManager.getLogger(AgronLogEntryManagement.class);
	...
	private static final String LASTRUNTIME_KEY =  LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.lastRunKey").trim();
	private static final String NAME_KEY = LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.nameKey").trim();
	private static final String STATUS_KEY =  LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.successKey").trim();
	private static final String MESSAGE_KEY =  LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.messageKey").trim();
	...
	public LCSLogEntry execute(String successOrFailure, String message) throws ParseException {
		...
		try {
			...
			LOGGER.info(dash);
			LOGGER.info(" Starting Data Export log entry Process  ");
			...
			logEntry=createLogEntry(message, currentDateTime, "ExportFile", successOrFailure);
	...
	public LCSLogEntry createLogEntry(String message, String currentTime, String logEntryName, String successOrFailure)
		...
		try {
			FlexType flextype = FlexTypeCache.getFlexTypeFromPath(LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.lcslogEntry"));
			...
			LOGGER.info("-------currentTimeStamp------>"+currentTimeStamp+", message-->"+message);
	...
	public String getLastSuccessfulRun() {
		...
		try {
			FlexType logEntryType = FlexTypeCache.getFlexTypeFromPath(LCSProperties.get("com.agron.wc.integration.straub.util.logEntry.lcslogEntry"));
