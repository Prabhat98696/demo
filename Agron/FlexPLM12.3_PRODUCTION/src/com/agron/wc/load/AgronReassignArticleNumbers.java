...
import com.agron.wc.product.AgronArticleNumberUPCPlugin;
...
public class AgronReassignArticleNumbers {
	...
	public static final Logger logger = LogManager.getLogger(AgronReassignArticleNumbers.class);
	...
	public AgronReassignArticleNumbers(WTUser userObj, String dataFile, String dataLoadType) {
	...
	public void loadData(ArrayList keyList, ArrayList dataList, boolean ignoreBlankValue) throws WTException, WTPropertyVetoException {
		...
		logger.info("***********Adidas Article Number Swap started...****************************");
		...
		while(dataListItr.hasNext()){
			...
			if(skuObjs.size() > 0) {
				...
				try{
						...
						if(ignoreBlankValue){
							if(!"".equals(value) && value!= null){
								if("0".equals(value)){
									...
									skuObject.setValue("agrArticle", (String)tempRow.get(j));
								}else{
					...
					//skuObject = new LCSProductLogic().saveSKU(skuObject);
					// CODE FOR TESTING STARTS
					// First update the SKU and unassign it.
				
					skuObject = new LCSProductLogic().saveSKU(skuObject);
					AgronArticleNumberUPCPlugin.setColumnNames();
					AgronArticleNumberUPCPlugin.findAndUpdateSKUSizes(skuObject);
					
					//skuObject.setValue("agrAdopted", true);
					//	skuObject = new LCSProductLogic().saveSKU(skuObject);
					//
	...
	public static void appendRowLoadStatus(StringBuffer logEntryLine, String status, String printMsg) {
		...
		logEntryLine.append(AgronReassignArticleNumbers.LOGGER_DELIMITER);
