...
public class AgronSKUSeasonConsolidatedBuyDateLoader {
	...
	public static final Logger logger = LogManager.getLogger(AgronSKUSeasonConsolidatedBuyDateLoader.class);
	...
	public AgronSKUSeasonConsolidatedBuyDateLoader(WTUser userObj, String dataFile, String dataLoadType) {
	...
	public void loadData(ArrayList keyList, ArrayList dataList, boolean ignoreBlankValue) throws WTException, WTPropertyVetoException {
		...
		logger.info("***********SKU SEASON Data upload started...****************************");
	 ...
	 if(dataList.isEmpty() && dataList.size() == 0) {
		...
		while(dataListItr.hasNext()){
			...
			//FlexType flexType_SKUP = product.getFlexType();
			//HashMap<String, String> criteria = new HashMap<String, String>();

			//criteria.put(flexType_SKUP.getAttribute("agrArticle").getSearchCriteriaIndex(), colorwayArticle);
			//criteria.put(flexType_SKUP.getAttribute("agrColorwayLetter").getSearchCriteriaIndex(), colorwayLetter);
						...
						if (ft.getAttVariableType().equals("boolean") && (!"".equals(tempRow.get(j).toString().trim()) || ft.isAttRequired() )) {
							...
							if(!bolAttribute.trim().equalsIgnoreCase("TRUE") && !bolAttribute.trim().equalsIgnoreCase("FALSE")){
								...
								if(isError) {
									...
									loggerRowData.append(AgronSKUSeasonConsolidatedBuyDateLoader.LOGGER_DELIMITER);
	...
	private void notifyPLMUser(boolean isValidFile,  ArrayList dataList, int successCount, int failedCount ) throws WTException {
		...
		if(isValidFile) {
			emailContent.append("Consolidated Date Data Load is successful.");
			}else {
				emailContent.append("Consolidated Date Data Load is failed.");
				emailContent.append("There is no data in the excel file So please add the data in the excel file");
			}
		String emailSubject = "Consolidated Date Upload Summary";
		emailContent.append(System.getProperty("line.separator"));
	...
	public static void appendRowLoadStatus(StringBuffer logEntryLine, String status, String printMsg) {
		...
		logEntryLine.append(AgronSKUSeasonConsolidatedBuyDateLoader.LOGGER_DELIMITER);
