...
public class AgronMinimumOrderQtyDataLoader {
	...
	public static final Logger logger = LogManager.getLogger(AgronMinimumOrderQtyDataLoader.class);
	public static String UPLOAD_ATTRIBUTE_LIST=LCSProperties.get("com.agron.wc.load.MinimumOrderQty.excelUploadAttributes");
	...
	public AgronMinimumOrderQtyDataLoader(WTUser userObj, String dataFile, String dataLoadType) {
	...
	public void loadData(ArrayList keyList, ArrayList dataList, boolean ignoreBlankValue) throws WTException, WTPropertyVetoException {
	 ...
	 if(dataList.isEmpty() && dataList.size() == 0) {
						...
						if (ft.getAttVariableType().equals("boolean") && (!"".equals(tempRow.get(j).toString().trim()) || ft.isAttRequired() )) {
							...
							if(!bolAttribute.trim().equalsIgnoreCase("TRUE") && !bolAttribute.trim().equalsIgnoreCase("FALSE")){
								...
								if(isError) {
									...
									loggerRowData.append(AgronMinimumOrderQtyDataLoader.LOGGER_DELIMITER);
		...
		logger.info("******Minimum Order Qty Data Load Summary**********");
	...
	private void notifyPLMUser(boolean isValidFile,  ArrayList dataList, int successCount, int failedCount ) throws WTException {
		...
		if(isValidFile) {
			emailContent.append("Minimum Order Qty Data Load is successful.");
			}else {
				emailContent.append("Minimum Order Qty Data Load is failed.");
				emailContent.append("There is no data in the excel file So please add the data in the excel file");
			}
		String emailSubject = "Minimum Order Qty Upload Summary";
		emailContent.append(System.getProperty("line.separator"));
	...
	public static void appendRowLoadStatus(StringBuffer logEntryLine, String status, String printMsg) {
		...
		logEntryLine.append(AgronMinimumOrderQtyDataLoader.LOGGER_DELIMITER);
